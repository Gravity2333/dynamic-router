...
export function createBrowserHistory({
  ...
  function getCurrentLocationAndIndex() {
    ...
    const formattedPathname = (pathname?.startsWith(basename)?pathname.slice(basename.length):pathname)
    return [
      readOnly<Location>({
        pathname: formattedPathname?.startsWith('/')?formattedPathname:'/'+formattedPathname,
        hash,
        search,
...
export function createHashHistory({
  window = document.defaultView!,
  basename = '',
}: { window?: Window; basename?: string } = {}): History {
  const globalHistory = window.history;
